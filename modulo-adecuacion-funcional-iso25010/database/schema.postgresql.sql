-- =============================================================================
-- Módulo Adecuación Funcional — ISO/IEC 25010
...
-- Evaluación de calidad (sesión de medición)
...
COMMENT ON TABLE qc_evaluaciones IS 'Sesiones de evaluación de Adecuación Funcional (ISO/IEC 25010)';

-- Completitud Funcional: funciones requeridas vs implementadas
...
-- Corrección Funcional: transacciones de negocio evaluadas
...
-- Vista de métricas (subconsultas — evita producto cartesiano en JOINs)
...
SELECT
    ...
    CASE
        WHEN (SELECT COUNT(*) FROM qc_funciones f WHERE f.evaluacion_id = e.id AND f.requerida) = 0 THEN NULL
        ELSE ROUND(
            100.0 * (SELECT COUNT(*) FROM qc_funciones f WHERE f.evaluacion_id = e.id AND f.requerida AND f.implementada)
            / (SELECT COUNT(*) FROM qc_funciones f WHERE f.evaluacion_id = e.id AND f.requerida),
            2
        )
    END AS completitud_funcional_pct,
    ...
    CASE
        WHEN (SELECT COUNT(*) FROM qc_transacciones_funcionales t WHERE t.evaluacion_id = e.id AND t.evaluada) = 0 THEN NULL
        ELSE ROUND(
            100.0 * (SELECT COUNT(*) FROM qc_transacciones_funcionales t WHERE t.evaluacion_id = e.id AND t.evaluada AND t.correcta)
            / (SELECT COUNT(*) FROM qc_transacciones_funcionales t WHERE t.evaluacion_id = e.id AND t.evaluada),
            2
        )
    END AS correccion_funcional_pct,
    ...
    CASE
        WHEN (SELECT COUNT(*) FROM qc_casos_prueba c WHERE c.evaluacion_id = e.id AND c.ejecutado) = 0 THEN NULL
        ELSE ROUND(
            100.0 * (SELECT COUNT(*) FROM qc_casos_prueba c WHERE c.evaluacion_id = e.id AND c.ejecutado AND c.aprobado)
            / (SELECT COUNT(*) FROM qc_casos_prueba c WHERE c.evaluacion_id = e.id AND c.ejecutado),
            2
        )
    END AS tecp_pct
...
-- RLS (solo service_role / admin vía BFF en producción)
