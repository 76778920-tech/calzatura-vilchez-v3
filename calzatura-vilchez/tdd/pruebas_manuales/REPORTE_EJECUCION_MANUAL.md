# Reporte De Ejecucion De Pruebas Manuales

## Datos Generales

| Campo | Valor |
| --- | --- |
| Proyecto | Calzatura Vilchez |
| Tipo de prueba | Manual |
| Responsable | __________________ |
| Fecha de ejecucion | __________________ |
| Ambiente | Local / Produccion / Staging |
| Navegador | Chrome / Edge / Firefox |
| Dispositivo | Desktop / Movil |

## Resumen De Ejecucion

| Estado | Cantidad |
| --- | ---: |
| Casos ejecutados | 0 |
| Casos aprobados | 0 |
| Casos fallidos | 0 |
| Casos bloqueados | 0 |
| Casos pendientes | 50 |

## Resultado Por Módulo

| Módulo | Total | Aprobados | Fallidos | Bloqueados | Pendientes |
| --- | ---: | ---: | ---: | ---: | ---: |
| Página principal | 3 | 0 | 0 | 0 | 3 |
| Catálogo de productos | 6 | 0 | 0 | 0 | 6 |
| Detalle de producto | 5 | 0 | 0 | 0 | 5 |
| Carrito | 5 | 0 | 0 | 0 | 5 |
| Registro | 4 | 0 | 0 | 0 | 4 |
| Login | 3 | 0 | 0 | 0 | 3 |
| Checkout | 5 | 0 | 0 | 0 | 5 |
| Pedido exitoso e historial | 3 | 0 | 0 | 0 | 3 |
| Perfil | 2 | 0 | 0 | 0 | 2 |
| Administración | 7 | 0 | 0 | 0 | 7 |
| Seguridad basica | 3 | 0 | 0 | 0 | 3 |
| Responsive | 4 | 0 | 0 | 0 | 4 |

## Registro De Ejecucion

| ID Caso | Resultado | Observacion | Evidencia | Tester |
| --- | --- | --- | --- | --- |
| PM-001 | Pendiente |  |  |  |
| PM-002 | Pendiente |  |  |  |
| PM-003 | Pendiente |  |  |  |
| PM-004 | Pendiente |  |  |  |
| PM-005 | Pendiente |  |  |  |
| PM-006 | Pendiente |  |  |  |
| PM-007 | Pendiente |  |  |  |
| PM-008 | Pendiente |  |  |  |
| PM-009 | Pendiente |  |  |  |
| PM-010 | Pendiente |  |  |  |
| PM-011 | Pendiente |  |  |  |
| PM-012 | Pendiente |  |  |  |
| PM-013 | Pendiente |  |  |  |
| PM-014 | Pendiente |  |  |  |
| PM-015 | Pendiente |  |  |  |
| PM-016 | Pendiente |  |  |  |
| PM-017 | Pendiente |  |  |  |
| PM-018 | Pendiente |  |  |  |
| PM-019 | Pendiente |  |  |  |
| PM-020 | Pendiente |  |  |  |
| PM-021 | Pendiente |  |  |  |
| PM-022 | Pendiente |  |  |  |
| PM-023 | Pendiente |  |  |  |
| PM-024 | Pendiente |  |  |  |
| PM-025 | Pendiente |  |  |  |
| PM-026 | Pendiente |  |  |  |
| PM-027 | Pendiente |  |  |  |
| PM-028 | Pendiente |  |  |  |
| PM-029 | Pendiente |  |  |  |
| PM-030 | Pendiente |  |  |  |
| PM-031 | Pendiente |  |  |  |
| PM-032 | Pendiente |  |  |  |
| PM-033 | Pendiente |  |  |  |
| PM-034 | Pendiente |  |  |  |
| PM-035 | Pendiente |  |  |  |
| PM-036 | Pendiente |  |  |  |
| PM-037 | Pendiente |  |  |  |
| PM-038 | Pendiente |  |  |  |
| PM-039 | Pendiente |  |  |  |
| PM-040 | Pendiente |  |  |  |
| PM-041 | Pendiente |  |  |  |
| PM-042 | Pendiente |  |  |  |
| PM-043 | Pendiente |  |  |  |
| PM-044 | Pendiente |  |  |  |
| PM-045 | Pendiente |  |  |  |
| PM-046 | Pendiente |  |  |  |
| PM-047 | Pendiente |  |  |  |
| PM-048 | Pendiente |  |  |  |
| PM-049 | Pendiente |  |  |  |
| PM-050 | Pendiente |  |  |  |

## Conclusion

Este reporte debe completarse manualmente mientras se ejecutan los casos definidos en la matriz.

Estados permitidos:

- Aprobada
- Fallida
- Bloqueada
- No aplica

