-- Hardening del modulo Admin Productos para cliente web con anon key.
-- Cubre crear variantes, editar, eliminar, ingreso de stock y lectura metadata.
...
GRANT SELECT ON TABLE "productoCodigos" TO anon;
GRANT SELECT ON TABLE "productoCodigos" TO authenticated;
GRANT SELECT ON TABLE "productoFinanzas" TO anon;
GRANT SELECT ON TABLE "productoFinanzas" TO authenticated;
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public'
      ...
      AND policyname = 'anon_select_productoCodigos'
  ) THEN
    CREATE POLICY "anon_select_productoCodigos"
      ON "productoCodigos"
      FOR SELECT
      TO anon
      USING (true);
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public'
      AND tablename = 'productoCodigos'
      AND policyname = 'authenticated_select_productoCodigos'
  ) THEN
    CREATE POLICY "authenticated_select_productoCodigos"
      ON "productoCodigos"
      FOR SELECT
      TO authenticated
      USING (true);
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public'
      ...
      AND policyname = 'anon_select_productoFinanzas'
  ) THEN
    CREATE POLICY "anon_select_productoFinanzas"
      ON "productoFinanzas"
      FOR SELECT
      TO anon
      USING (true);
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public'
      AND tablename = 'productoFinanzas'
      AND policyname = 'authenticated_select_productoFinanzas'
  ) THEN
    CREATE POLICY "authenticated_select_productoFinanzas"
      ON "productoFinanzas"
      FOR SELECT
      TO authenticated
      USING (true);
...
BEGIN
  ...
  IF to_regclass('public."movimientosStock"') IS NOT NULL THEN
    ...
    '
    USING p_product_id, 'ingreso', p_talla_stock, v_cantidad, p_costo_unitario,
      p_proveedor, p_observaciones, p_registrado_por;
  END IF;
...
CREATE OR REPLACE FUNCTION delete_product_atomic(p_id text)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
BEGIN
  DELETE FROM "productoFinanzas" WHERE "productId" = p_id;
  DELETE FROM "productoCodigos" WHERE "productoId" = p_id;
...
REVOKE ALL ON FUNCTION delete_product_atomic(text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION delete_product_atomic(text) TO anon;
GRANT EXECUTE ON FUNCTION delete_product_atomic(text) TO authenticated;
