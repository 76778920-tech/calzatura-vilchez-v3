...
describe("Carrito sessionStorage guards", () => {
  it("persiste en sessionStorage sin clave por UID", () => {
    ...
    expect(cartContextSource).toContain('const CART_AUTH_SESSION_PREFIX = `${CART_AUTH_SESSION_KEY}:`');
    expect(cartContextSource).toContain("cartSessionHash(userUid)");
  ...
  it("limpia carrito de sesion auth solo al cambiar entre dos cuentas", () => {
    ...
    expect(cartContextSource).toContain("sessionStorage.removeItem(activeCartStorageKey(prevUid))");
  });
