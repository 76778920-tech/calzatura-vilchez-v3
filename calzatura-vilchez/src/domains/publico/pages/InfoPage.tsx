...
const LEGAL_PROSE_PAGE_KEYS = new Set<string>([
  "terminos",
  "privacidad",
  "politicaCookies",
  "libroReclamaciones",
...
export default function InfoPage({ pageKey }: Props) {
  ...
  return (
    <main className={`info-page info-page--${content.group}`}>
      ...
      <div
        className={`info-page-main${LEGAL_PROSE_PAGE_KEYS.has(pageKey) ? " info-page-main--prose" : ""}`}
      >
        {LEGAL_PROSE_PAGE_KEYS.has(pageKey) ? (
          <article className="info-page-prose" aria-label={content.title}>
            {content.sections.map((section) => (
              <section key={section.title} className="info-page-prose-section">
                ...
                {pageKey === "libroReclamaciones" &&
                section.title.startsWith("5. Presentar") ? (
                  <ComplaintBookPanel />
                ) : null}
              </section>
            ))}
            {pageKey === "politicaCookies" ? (
              <section className="info-page-prose-section">
                ...
                <p>
                  La siguiente relación complementa las secciones anteriores y tiene carácter
                  informativo. No sustituye el panel interactivo de «Configuración de cookies»,
                  donde puedes activar o desactivar las categorías que requieren consentimiento.
                  Los nombres comerciales indicados buscan facilitar tu comprensión y no constituyen
                  una lista exhaustiva de identificadores técnicos internos.
                </p>
                <CookiePolicyProse />
              </section>
            ) : null}
      ...
      {content.faq && content.faq.length > 0 && !LEGAL_PROSE_PAGE_KEYS.has(pageKey) ? (
        <section
          className={`info-page-faq${LEGAL_PROSE_PAGE_KEYS.has(pageKey) ? " info-page-faq--prose" : ""}`}
          aria-label="Preguntas frecuentes de la página"
        >
          <h2>Preguntas frecuentes</h2>
          {LEGAL_PROSE_PAGE_KEYS.has(pageKey) ? (
            <div className="info-page-faq-prose">
