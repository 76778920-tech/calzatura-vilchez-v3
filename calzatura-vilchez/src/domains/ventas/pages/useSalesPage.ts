...
export type SalesPageConfig = Readonly<{
  /** Alcance BFF fijo: admin → /admin/* ; staff → /staff/* (sin mezclar por rol en runtime). */
  financeScope: FinanceFetchScope;
  showFinancialDetails: boolean;
}>;

export function useSalesPage(config: SalesPageConfig): SalesPageModel {
  const { financeScope, showFinancialDetails } = config;
  const panel = financeScope;
  ...
  const load = useCallback((targetDate = date) => {
    ...
    return Promise.allSettled([
      fetchProducts(financeScope),
      fetchProductCodes(financeScope),
      financialsPromise,
      fetchDailySales(targetDate, financeScope),
    ])
  ...
  const addLine = () => {
    ...
    if (!selectedProduct?.finanzas) {
      toast.error(
        financeScope === "admin"
          ? "Este producto no tiene costo ni rango de venta registrado"
          : "Este producto no tiene rango de precio configurado",
      );
      return;
    }
  ...
  const registerPendingLines = () => {
    void executeRegisterPendingSales({
      pendingLines,
      products,
      date,
      documentType,
